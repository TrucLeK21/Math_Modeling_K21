...
def shortest_paths(n, v0, adj, cost, capacity):
    ...
    d[v0] = 0  # Set the distance to the source node as 0
    inq = [False] * n  # Keep track of nodes in the queue
    q = deque([v0])  # Initialize queue with the source node
    p = [-1] * n  # Predecessor array to reconstruct the path
    ...
    while q:
        ...
        for v in adj[u]:  # Iterate through neighboring nodes
            ...
            if capacity[u][v] > 0 and d[v] > d[u] + cost[u][v]:
                d[v] = d[u] + cost[u][v]  # Update distance
...
plt.figure(figsize=(14, 6))  # Set your desired width and height in inches
